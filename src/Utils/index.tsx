...
// 过万点赞数显示
function transformCommNum(num: number): number|string {
...
function getWetimeHost(env: string, app: string = 'api'): string {
  ...
  if (env === 'php') {
    ...
    if (host.indexOf('dev') > -1) return HostObj.dev
    if (host.indexOf('test') > -1) return HostObj.test
    if (host.indexOf('pre') > -1) return HostObj.pre
...
export function getFancylifeHost(): string {
  const host = window.location.host
  const protocol = window.location.protocol
  const HostObj = {
  ...
  if (host.indexOf('sit') > -1) return HostObj.test
  if (host.indexOf('pre') > -1) return HostObj.pre
  return HostObj.online
}

export function getMapiHost(): string {
  const host = window.location.host
  const protocol = window.location.protocol
  const HostObj = {
...
export function getMeitunHost(): string {
  const host = window.location.host
  const protocol = window.location.protocol
  const HostObj = {
...
// 视频时间格式转化
function formatMiniutes(time: number): string {
  let second = 0
  let miniutes = 0
  let _second = ''
  let _miniutes = ''
...
// 新版评论时间显示
export function commentTimeTransform(time: string): string {
  if (isToday(time)) {
