...
function getQuality(userQuality) {
    let resultQuality;
...
function adjustImgUrl(srcUrl, finalW, finalH, config) {
    ...
    const rawSrc = srcUrl.replace(/_\d+x\d+(q\d+)?\.jpg/g, '').replace(/_q\d+\.jpg/g, '').replace(/_\.webp/, ''), ignoreWidth = (config && config.ignoreWidth), ignoreHeight = (config && config.ignoreHeight);
    let finalSrc, cdnW, cdnH;
    // 寻找最匹配的宽高
...
class DatalazyPlugin {
    ...
    dealLazyObj(obj) {
        if (obj.type !== 'img' || !obj.elem || !obj.src || !/http/.test(obj.src) || hasAttribute(obj.elem, 'crossimage-ignore'))
            return;
        ...
        try {
            const imgEle = obj.elem, expectW = imgEle.width * this.config.userPPI, expectH = imgEle.height * this.config.userPPI, currentSrc = obj.src;
